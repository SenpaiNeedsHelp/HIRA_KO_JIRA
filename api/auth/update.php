...
try {
    ...
    if ($updateSettings) {
        ...
        $weeklyReportTime = isset($settings['weekly_report_time']) ? $settings['weekly_report_time'] : '09:00:00';
        $timezone = isset($settings['timezone']) ? trim($settings['timezone']) : 'UTC';
        $theme = isset($settings['theme']) ? trim($settings['theme']) : 'auto';

        $settingsQuery = "INSERT INTO user_settings (user_id, theme, notifications_enabled, weekly_report_enabled, notification_time, weekly_report_time, timezone)
                          VALUES (:user_id, :theme, :notifications_enabled, :weekly_report_enabled, :notification_time, :weekly_report_time, :timezone)
                          ON DUPLICATE KEY UPDATE
                            theme = VALUES(theme),
                            notifications_enabled = VALUES(notifications_enabled),
                            weekly_report_enabled = VALUES(weekly_report_enabled),
                            notification_time = VALUES(notification_time),
                            weekly_report_time = VALUES(weekly_report_time),
        ...
        $settingsStmt->bindParam(':weekly_report_time', $weeklyReportTime);
